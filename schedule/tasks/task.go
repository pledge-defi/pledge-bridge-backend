...
func Task() {
	...
	//_=scheduler.Every(1).Sunday().At("00:00").Do(services.NewBridgeService().BridgeUpKeep, config.Config.BscNet.NetUrl, config.Config.BscNet.BridgeToken, common.BridgeBscAdminPrivateKey, config.Config.BscNet.ChainId) // Begin job immediately upon start
